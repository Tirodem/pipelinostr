...
async function handleQueue(sub: string | undefined, args: string[]): Promise<void> {
  ...
  switch (sub) {
    ...
    case 'clear': {
      const sqliteDb = (storage as unknown as Record<string, unknown>)['db'] as { prepare: (sql: string) => { run: () => { changes: number } } } | undefined;
      if (sqliteDb?.prepare) {
        const r = sqliteDb.prepare('DELETE FROM queue').run();
        console.log(`  Cleared ${r.changes} queue entries.`);
      } else {
        console.error('  Cannot access database directly.');
      }
      break;
    }
    default:
